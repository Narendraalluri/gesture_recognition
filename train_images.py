...
def process_file(label, file):
    ...
    lines = f.readlines()
    last_line = lines[-1]
    result_list = [word.split(' ') for word in last_line.split(',')]
    result = np.array(result_list)
    min1st = min(result.shape[0],padded.shape[0])
    min2nd = min(result.shape[1],padded.shape[1])
    padded[:min1st ,:min2nd] = result[:min1st ,:min2nd]
    return (padded, label)
...
def main():
    ...
    labels = [ f.name for f in os.scandir('data/images') if f.is_dir() ]
    ...
    result = [ process_folder(f) for f in os.scandir('data/images') if f.is_dir() ]
